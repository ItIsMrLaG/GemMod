...
def setup_grid():
    ...
    _CELL_SZ = RADIUS_1
    _GRID_COL_N = math.ceil(PLATE_WIDTH / _CELL_SZ)
    _GRID_ROW_N = math.ceil(PLATE_HEIGHT / _CELL_SZ)
    _CELL_N = _GRID_COL_N * _GRID_ROW_N

    global _F_CELL_HEADS, _F_CELL_STORAGE
    _F_CELL_STORAGE = ti.field(dtype=ti.i32, shape=(CATS_N,))
        ...
        for cell_col in range(x_begin, x_end):
            for cell_row in range(y_begin, y_end):
                ...
                for _idx2 in range(_F_CELL_HEADS[neighbour_lin_idx],
                    ...
                    if idx1 != idx2:
                        cats[idx1].fight_with(cats[_F_CELL_STORAGE[idx2]], distance_type)
