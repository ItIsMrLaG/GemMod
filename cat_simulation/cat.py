...
class Cat:
    ...
    @ti.func
    def _update_status(self, status: ti.i32):
        self.status = status
    ...
    @ti.func
    def move(self):
        self._update_status(INTERACTION_NO)
        if self.move_pattern == MOVE_PATTERN_RANDOM_ID:
            self.prev_point = tm.vec2([self.point[0], self.point[0]])
    ...
    def fight_with(self, other_cat: ti.template(), distance_type: ti.i32):
        ...
        elif dist <= RADIUS_1:  # and ti.round(ti.random() * (1 / (dist * dist)), dtype=ti.i32) == 1
            self_status = ti.max(self.status, INTERACTION_LEVEL_1)

        else:
            self_status = INTERACTION_NO

        self._update_status(self_status)
