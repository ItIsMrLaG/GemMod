...
> Библиотека `taichi` накладывает ряд ограничений, из-за которых было удобно воспринимать один модуль-файл, как один
> класс.
> Кроме того, для ускорения работы алгоритма, вся динамическая память выделяется заранее на этапе конфигурации и потом
> переиспользуется.
>
> АЛЬТЕРНАТИВА: можно было бы выделять память при каждом запуске функции, однако такой подход очевидным образом не
> эффективен, так как приходится тратить время на выделение памяти.
...
* `config.py` -- внутри содержится класс Config, отвечающий за парсинг и файла конфигурации и представление настроек
  запуска
...
* Этого достаточно, потому что высота и ширина ячейки равняется `2*ACT_MAX_RADIUS`.

> Важно заметить, что в худшем случае, когда `ACT_MAX_RADIUS` будет достаточно большим, весь алгоритм будет работать за
`O(n^2)`,
...
* Тестирование алгоритма ячеек проводилось путем сравнения его результатов с "наивным" решением, основанным на методе
  полного
  перебора. "Наивное" решение подразумевает исчерпывающий поиск соседних узлов _("котов")_ для каждого элемента набора
  данных путем
...
* Также проведено юнит-тестирование функций перемещения: протестировано, что функции не нарушают допустимый радиус
  перемещения.
* Были добавлены тесты, проверяющие корректность инварианта появления и исчезновения котов (что если кот вышел за
  границу поля, то появится он только тогда, когда попадет на границу + будет существовать путь из точки исчезновения в
  точку появления за границами поля)

## Конфигурация
Был обновлен способ конфигурации приложения. Если раньше использовались .py файлы,
в которых были прописаны через глобальные переменные установленные настройки, то теперь используются .json файлы.
Такой подход является более гибким и защищенным:
* позволяет неявно конфигурировать дефолтные параметры;
* отделяет пользовательский интерфейс от реализации;
* упрощает изменение настроек;
* позволяет разграничивать публичные (доступные пользователю) и приватные настройки.

Кроме того валидация пользовательской конфигурации была также перемещена из main.py в класс Config.
